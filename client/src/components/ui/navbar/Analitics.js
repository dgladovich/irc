...
export default View.extend({
  ...
  showChart(d) {
    const chronology = this.anal(this.model.get('history_events'));
  ...
  updateAnalitics(analitics) {
    this.model.set('history_events', analitics.history_events);
    const {
      worktime_count, errortime_delay, crashes_count, load_percent,
    } = analitics;
    this.$('#anal').html(anal);
  ...
  onRender() {
    const device = app.devices.findWhere({ parent: 0 });
    this._requestAnalitics().then((data) => {
      const anal = this.analTemplate(data);
      this.$('#anal').html(anal);
    });
  },
});
