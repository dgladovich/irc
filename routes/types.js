          ...
          promises.push( Device.findAll({ where: { typ: type.get('id') } }).then(( devices )=>{
            if(devices.length > 0){
              ...
              devices.forEach(( device )=>{
                ...
                DeviceService
                  .findOrCreate({ where: { ser_num: 1, dev: device.get('id') }, defaults: { dev: device.get('id'), ser_num: 1, service_id: type.get('services')[0].id, _lim: 111 } })
                  .spread((deviceService, created)=>{ 
                    ...
                    Work.findAll({ where: { to_id: type.services[0].id } }).then(( works )=>{
                      if( works.length > 0 ){
                        works.forEach(( work )=>{
                          DeviceServiceWork.findOrCreate({ where: { dev: device.get('id'), work_id: work.get('id') }, defaults: { dev: device.get('id'), ser_num: 1, work_id: work.get('id'), done: 0, service_id: deviceService.get('id') } })
                ...
                DeviceService
                  .findOrCreate({ where: { ser_num: 2, dev: device.get('id') }, defaults: { dev: device.get('id'), ser_num: 2, service_id: type.get('services')[1].id, _lim: 222 } })
                  .spread((deviceService, created)=>{ 
                    console.log('DeviceService ID is: ',deviceService.get('id'), '+++++++++++++++++++++++++++++++=');
                ...
                DeviceService
                  .findOrCreate({ where: { ser_num: 3, dev: device.get('id') }, defaults: { dev: device.get('id'), ser_num: 3, service_id: type.get('services')[2].id, _lim: 333 } })
                  .spread((deviceService, created)=>{ 
                    console.log('DeviceService ID is: ',deviceService.get('id'), '+++++++++++++++++++++++++++++++=');
                ...
                DeviceService
                  .findOrCreate({ where: { ser_num: 4, dev: device.get('id') }, defaults: { dev: device.get('id'), ser_num: 4, service_id: type.get('services')[3].id, _lim: 666 } })
                  .spread((deviceService, created)=>{ 
                    console.log('DeviceService ID is: ',deviceService.get('id'), '+++++++++++++++++++++++++++++++=');
